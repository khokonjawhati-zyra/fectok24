            ...
            if auth_data.get("email_phone") == normalized_id:
                if auth_data.get("password") == hashed_pw:
                    ...
                    if sov_id in self.users:
                        ...
                        # Sovereign V15: High-Trust OTP Protocol [Enabled by Admin]
                    otp = str(random.randint(100000, 999999))
                    self.login_pulses[normalized_id] = {
                        "sov_id": sov_id,
                        "otp": otp,
                    ...
                    import threading
                    threading.Thread(target=self.send_auth_otp, args=(normalized_id, otp, name)).start()
                    
                    logger.info(f"LOGIN_OTP: Challenge Sent to {normalized_id}")
