...
class ConnectionManager:
    ...
    def _normalize_url(self, url, request: Request = None):
        ...
        # 1. Detect if it's an internal resource (IP, domain, or relative)
        is_internal = ("167.71.193.34" in url) or ("fectok.com" in url) or (not url.startswith("http"))
        
        if is_internal:
            # 1. Strip protocol/host
            p = re.sub(r'https?://[^/]+', '', url)
            
            # 2. Aggressively strip leading slashes and 'stream/' chunks
            p = p.lstrip('/')
            while p.startswith('stream/'):
                p = p[7:].lstrip('/')
                
            # 3. Return canonical V15 root-relative path
            return f"/stream/{p}"
