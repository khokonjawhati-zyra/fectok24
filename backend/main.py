...
from gmail_engine_brevo import gmail_engine # Sovereign V15: Gmail API Pulse
...
class user_manager:
    def __init__(self):
        ...
        self.ADMIN_MASTER_KEY = os.getenv("ADMIN_MASTER_KEY")
        ...
        # V15 Private Email Gateway Configuration (SMTP-Free HTTP Protocol)
        self.BREVO_API_KEY = os.getenv("BREVO_API_KEY")
        self.SENDER_EMAIL = os.getenv("SENDER_EMAIL", "noreply@fectok.com")
        self.ADMIN_TARGET_EMAIL = os.getenv("ADMIN_TARGET_EMAIL")
        
        # V15 Quantum Security Protocol
        self.LEDGER_SECRET = os.getenv("LEDGER_SECRET")
        self.admin_pulses = {} # HWID -> {"otp": str, "expiry": datetime}
        self.admin_failed_nodes = {} # IP/HWID -> {"count": int, "lock": datetime}
            ...
            if auth_data.get("email_phone") == normalized_id:
                if auth_data.get("password") == hashed_pw:
                    ...
                    if sov_id in self.users:
                        ...
                        # Sovereign V15: Direct Access Pulse [User Override: No 2-Step]
...
async def admin_auth_init(req: dict, request: Request):
    ...
    if master_clean == user_auth.ADMIN_MASTER_KEY and pin_clean == user_auth.ADMIN_PIN and hwid == user_auth.ADMIN_HWID:
        ...
        import threading
        threading.Thread(target=user_auth.send_admin_otp, args=(user_auth.ADMIN_TARGET_EMAIL, otp)).start()
