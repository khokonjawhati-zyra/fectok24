...
def stability_patch_ignition():
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(HOST, username=USER, password=PASS, timeout=15)
    
    # Master Stability DNA for Nginx
    STABILITY_CONF = """
user  nginx;
...
"""
    ...
    with sftp.file('/root/sovereign/nginx.conf.gateway', 'w') as f:
        f.write(STABILITY_CONF)
    sftp.close()
    
    # FINAL RE-IGNITION
    ssh.exec_command('docker restart sovereign_v15_gateway')
    print("✅ DNA Pulse Synchronized: Handshake Stability IS LIVE.")
    ssh.close()

if __name__ == "__main__":
    stability_patch_ignition()
