...
try:
    ...
    class DummyGovernor:
        ...
        async async def close(self): pass
...
async def host_ui_middleware(request: Request, call_next):
    ...
    if path.startswith("/api/") or path.startswith("/ws/") or path.startswith("/media/") or path.startswith("/stream") or path in ["/admin_auth_init", "/admin_auth_verify", "/login", "/forgot_password", "/reset_password", "/register", "/verify_token", "/reset_pulse", "/resend_otp"]:
...
class user_manager:
    def __init__(self):
        ...
        self.SMTP_PORT = 587
        self.ADMIN_TARGET_EMAIL = "khokonjawhati@gmail.com"
    ...
    def send_auth_otp(self, email: str, code: str, name: str):
        ...
        logger.info(f"DEBUG: Inside send_admin_otp for email={email}")
