...
def pure_docker_restoration_v15():
    print("--- 1. CONNECTING TO SOVEREIGN CORE ---")
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(HOST, username=USER, password=PASS, timeout=30)
    
    print("\n--- 2. EMERGENCY GATEWAY RECOVERY ---")
    # Nuking existing complex compose and nginx files
    ssh.exec_command("rm -f /root/sovereign_v15/docker-compose.yml /root/sovereign_v15/nginx.conf")

    print("\n--- 3. INJECTING GUARANTEED DOCKER-COMPOSE ---")
    # A fresh, error-free docker-compose mapping Port 80
    compose_simple = """
version: '3.8'
services:
...
"""
    ...
    ssh.close()
    print("\n--- MISSION STATUS: PURE DOCKER RESTORATION COMPLETE ---")

if __name__ == "__main__":
    pure_docker_restoration_v15()
