...
http {
    ...
    # Port 443 remains if user switches to Full SSL later
    server {
...
def deploy_nginx_final_final():
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(HOST, username=USER, password=PASS, timeout=15)
    
    print("Writing Nginx config to handle API on both ports (Cloudflare Friendly)...")
    sftp = ssh.open_sftp()
